...
"""
    ...
    def cmd_analyze_code(self, args):
        "Generate summaries for files in the repository map"

        switchOptions = args.strip().lower()

        if not self.coder.repo_map:
            self.io.tool_error("Repo map is not initialized.")
            return

        if switchOptions == "files":
            self._analyze_code_files()
        elif switchOptions == "files-force":
            self._analyze_code_files(True)
        elif switchOptions == "dir":
            self._analyze_code_dir()
        elif switchOptions == "dir-force":
            self._analyze_code_dir(True)
        elif switchOptions == "arch":
            self._analyze_code_arch()
        elif switchOptions == "arch-force":
            self._analyze_code_arch(True)
        else:

...
class Coder:
    ...
    def get_directories(self):
        """Return a list of directories within the given path."""
        directories = set()
    ...
    def get_platform_info(self):
        ...
        if self.test_cmd:
            if self.auto_test:
                platform_text += (
                    "- The user's pre-commit runs this test command, "
                    "don't suggest running them: "
                )
            else:
    ...
    def send(self, messages, model=None, functions=None):
        ...
        if self.main_model.use_temperature:
            ...
            # temp = self.temperature if self.temperature > 0 else 0.2
        else:
            temp = None
